...
export class LrelevetComponent implements OnInit {
  ...
  public openPDF(ref): void {
    this.getsumrelevet(this.refrelevet).subscribe((bon: any) => {

      //console.log(this.date_facture);
      const months = ["January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December"];
      let date_r=new Date(this.date_facture);
      let doc = new jsPDF('p', 'mm', 'a4');
      doc.setFontSize(12);
      doc.setFont("Times-Roman");
      doc.text("Nous avons l’honneur de vous adresser ci-joints nos factures ainsi que vos bons joints aux vignettes correspondantes. ", 23, 80,{maxWidth:170,lineHeightFactor:2.1});
      doc.text("Vous trouverez ci-joint aussi le bordereau de facturation concernant le mois de "+months[date_r.getMonth()], 23, 100,{maxWidth:170});
      doc.text(date_r.getFullYear()+" . Ce bordereau fait ressortir un montant de "+Number(Number(bon[0].total_brut)-Number(bon[0].remise)).toFixed(2)+" Dhs", 23, 110,{maxWidth:170});
      doc.text("En vous souhaitant bonne réception, nous vous prions d’agréer l’expression de nos", 23, 120,{maxWidth:170});
      doc.text("sentiments les plus distingués.", 23, 130,{maxWidth:170});
      doc.setFont("Arial", "bold");
      doc.setFontSize(14);
      doc.text("Pièces jointes : Factures + vignettes + bons", 23, 170,{maxWidth:170});
      doc.text("Montant brut : "+bon[0].total_brut.toFixed(2)+" Dhs", 23, 180,{maxWidth:170});
      doc.text("Remise : "+bon[0].remise.toFixed(2)+" Dhs", 23, 190,{maxWidth:170});
      doc.text("Montant relevé : "+Number(Number(bon[0].total_brut)-Number(bon[0].remise)).toFixed(2)+" Dhs", 23, 200,{maxWidth:170});
      doc.text("Montant rejets : ", 23, 210,{maxWidth:170});
      doc.text("Montant réceptionné :", 23, 220,{maxWidth:170});
      doc.setFontSize(12);
      doc.setFont("Times-Roman","normal");
      doc.text("Nous vous prions d’agréer l’expression de nos salutations les plus distinguées.", 23, 250,{maxWidth:170});
      doc.save(ref+".pdf");
    });
    this.closePopuppdf();
  }
  async onUpload(): Promise<void> {
    const invoiceFile = await this.onDownloadReleve();
    //console.log(this.numero_facture);
    var allbons: Array<any> = [];
    var num_rel=this.refrelevet;
    var total_bon = 0, total_brut = 0, total_remise = 0, total_net = 0;
    this.getfactureallbons(this.refrelevet).subscribe((bon: Array<any>) => {
      allbons = bon;
    })
      ...
      if (reader.result instanceof ArrayBuffer) {
        ...
        wb.xlsx.load(reader.result)
          .then(function () {
            ...
            allbons.forEach(el => {
              var jour = el.date_facture.toString().slice(8, 10);
              var mois = el.date_facture.toString().slice(6, 7);
              var annee = el.date_facture.toString().slice(0, 4);
              //console.log(jour)
              worksheet.getCell('B' + i).value = el.refefact;
              worksheet.getCell('C' + i).value = jour.padStart(2,'0')+" / "+mois.padStart(2,'0')+" / "+annee.padStart(4,'0');
              worksheet.getCell('D' + i).value = "";
              worksheet.getCell('E' + i).value = el.total_bon;
              worksheet.getCell('F' + i).value = el.total_brut.toFixed(3);
              worksheet.getCell('G' + i).value = (Number(el.total_brut)-Number(el.remise)).toFixed(3);
              worksheet.getCell('H' + i).value = el.remise.toFixed(3);
              total_bon += Number(el.total_bon);
              total_brut += Number(el.total_brut);
              total_remise += Number(el.remise);
              total_net += Number(el.total_brut) - Number(el.remise);
              i++;
            })
            worksheet.getCell('E38').value = Number(total_bon).toFixed(3);
            worksheet.getCell('F38').value = Number(total_brut).toFixed(3);
            worksheet.getCell('H38').value = Number(total_remise).toFixed(3);
            worksheet.getCell('G38').value = Number(total_net).toFixed(3);
            ...
            wb.xlsx.writeBuffer().then((data) => {
              ...
              fetch(url)
                ...
                .then(blob => {
                  ...
                  link.download = "relevet_"+num_rel+".xlsx";
                  link.click();
                })
                .catch(console.error);
            })
  ...
  downloadObjectAsJson() {
    ...
    var mont_t: number = 0;
    var numfacture;
    this.getfactureallbons(this.refrelevet).subscribe((bon: Array<vignette>) => {
      //console.log(bon);
      bon.forEach(el => {
        var m1 = "", m2 = "";
        var matricule = "";
        var q = (Number(Number(el.quantite).toFixed(2)) * 100).toFixed(0).toString().padStart(6, '0');
        var pu = (Number(Number(el.prixunit).toFixed(2)) * 100).toFixed(0).toString().padStart(6, '0');
        ...
        mont_t += Number((Number(Number(el.quantite) * Number(el.prixunit)) - ((Number(el.quantite) * Number(el.prixunit)) * 0.005)).toFixed(2));
      });
      //console.log((mont_t*100).toString().padStart(9,'0'));
      data += '1' + codef.padStart(2, '0') + reff.padStart(6, '0') + '082022' + "                                                                      " + (mont_t * 100).toFixed(0).toString().padStart(9, '0');
      //console.log(data);
      let exportname = "relevet-"+numfacture;
  ...
  getAllfacture(): Observable<any> {
    return this.httpClient.get<any>('http://164.68.124.58:3000/facture/allrelevet', {
  ...
  getsumrelevet(idrel:number): Observable<any> {
    return this.httpClient.get<any>('http://164.68.124.58:3000/facture/allrelevet/calc/'+idrel, {
      headers: new HttpHeaders(
        {
          'Access-Control-Allow-Origin': '*',
          'Access-Control-Allow-Methods': 'GET',
  ...
  getfactureallbons(id: number): Observable<any> {
    return this.httpClient.get<any>('http://164.68.124.58:3000/facture/allrelevet/' + id, {
      headers: new HttpHeaders(
        {
          'Access-Control-Allow-Origin': '*',
          'Access-Control-Allow-Methods': 'GET',
  ...
  devaliderfacture_button() {
    this.devaliderfacture(this.id_delete).subscribe((bon: any) => {
      console.log(bon);
    });
    this.closePopup();
